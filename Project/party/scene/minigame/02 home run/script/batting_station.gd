...
const PITCH_LENGTHS : Array[float] = [1.0, 2.0, 1.5]
...
const HIT_TRAVEL_LENGTH : float = 2.0
## How high the ball should fly when hit.
const HIT_HEIGHT : float = 100.0
...
const CATAPULT_ANIMATION_MULTIPLIER : float = 1.5
## Multiplier for the swing animation, because it feels sluggish otherwise.
const SWING_ANIMATION_MULTIPLIER : float = 1.8
...
const CPU_MAX_VARIANCE : float = 0.1;
...
func launch_ball() -> void:
	...
	ball_speed = calculate_travel_speed(PITCH_LENGTHS[0]) # TODO Use a randomly generated index that is synced across the network
	travel_ratio = 0.0
...
func calculate_swing_ratio() -> void:
	...
	if player_index != -1:
		# Add randomness based on CPU difficulty
		swing_offset += calculate_cpu_difficulty_offset()
...
func calculate_cpu_difficulty_offset() -> float:
	...
	print("Offset %s by %s." % [player_index, offset])
	return offset
